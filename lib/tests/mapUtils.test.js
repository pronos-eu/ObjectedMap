...
const objectedMap_1 = require("../src/objectedMap");
const mapUtils_1 = require("../src/mapUtils");
...
describe(`ObjectedMap should create Map object with support of object as keys`, () => {
    ...
    const map = new objectedMap_1.ObjectedMap(['key1', 'key2']);
    ...
    test(`Should properly set and return object as key (without reference) when using full object as keys`, () => {
        testCases.forEach(testCase => {
            expect(map.get((0, testUtils_1.cloneObject)(testCase))).toEqual(testCase);
        });
    });
...
describe(`createdGroupedObjectedMap should properly map and group array of objects`, () => {
    ...
    const groupedMap = (0, mapUtils_1.createdGroupedObjectedMap)(objectsWithNotUniqueIndexKeys, ['key1', 'key2']);
    ...
    test(`Should properly return grouped objects`, () => {
        expect(groupedMap.get({ key1: 'value1a', key2: 'value2a' })).toEqual([(0, testUtils_1.cloneObject)(testCases[0]), (0, testUtils_1.cloneObject)(testCases[1])]);
        expect(groupedMap.get({ key1: 'value2b', key2: 'value2c' })).toEqual([(0, testUtils_1.cloneObject)(testCases[2]), (0, testUtils_1.cloneObject)(testCases[3])]);
        expect(groupedMap.get({ key1: 'value2c', key2: 'value2d' })).toEqual([(0, testUtils_1.cloneObject)(testCases[4])]);
        expect(groupedMap.get({ key1: 'value2c', key2: 'value2dx' })).toEqual([(0, testUtils_1.cloneObject)(testCases[5])]);
    });
    test(`Should be instance of ObjectedMap`, () => {
        expect(groupedMap).toBeInstanceOf(objectedMap_1.ObjectedMap);
    });
});
describe(`createObjectedMap should properly create ObjectedMap from array from given index keys`, () => {
    const testCases = objectsWithUniqueIndexKeys;
    const map = (0, mapUtils_1.createObjectedMap)(testCases, ['key1', 'key2']);
    test(`Should have only ${testCases.length} keys set`, () => {
        expect(map.size).toBe(testCases.length);
    });
    test(`Should be instance of ObjectedMap`, () => {
        expect(map).toBeInstanceOf(objectedMap_1.ObjectedMap);
    });
});
//# sourceMappingURL=mapUtils.test.js.map